...
([nodewright#296](https://github.com/NVIDIA/nodewright/issues/296)). Because
...
Bundling an AKS recipe without one emits a deployment-notes warning
(`CheckWildcardAcceleratedToleration`).

To opt out of the RDMA stack entirely (e.g., on non-InfiniBand SKUs):
